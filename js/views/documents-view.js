...
const DocumentsView = {
    ...
    if (isEmployee) {
      docs = docs.filter(
        (d) =>
          ...
          (d.employeeName &&
            d.employeeName.toLowerCase() === currentEmpName.toLowerCase()) ||
          (d.categoryCode === "COMPANY" &&
            (!d.employeeId || d.employeeId === "ALL")),
      );
      requests = requests.filter(
        (r) =>
          r.employeeId === currentEmpId ||
          (r.employeeName &&
            r.employeeName.toLowerCase() === currentEmpName.toLowerCase()),
      );
      expiringDocs = expiringDocs.filter(
        (d) =>
          d.employeeId === currentEmpId ||
          (d.employeeName &&
            d.employeeName.toLowerCase() === currentEmpName.toLowerCase()),
  ...
  renderAllDocumentsTab(docs) {
    ...
    if (this.currentFilters.categoryCode !== "All") {
      list = list.filter(
        (d) => d.categoryCode === this.currentFilters.categoryCode,
      );
    }
      ...
      const companyPolicies = [
        {
          ...
          category: "HR Policy",
          desc: "Standards of professional behavior, anti-bribery, conflicts of interest, and organizational integrity.",
          version: "v2.4 (2026)",
        ...
        },
        {
          ...
          category: "Operations",
          desc: "Working hours, biometric/web punch guidelines, earned leave accruals, comp-off rules, and WFH quotas.",
          version: "v3.1 (2026)",
          date: "Updated Feb 2026",
        },
        {
          title: "IT Security, Asset Custody & Data Privacy",
          category: "IT & Security",
          desc: "Acceptable hardware use, data classification, password rotation, VPN compliance, and IP protection.",
          version: "v2.0 (2025)",
          date: "Updated Dec 2025",
        },
        {
          title: "POSH & Workplace Harassment Redressal Policy",
          category: "Compliance",
          desc: "Prevention of Sexual Harassment (POSH) framework, reporting mechanisms, and Internal Committee contacts.",
          version: "v1.5 (2026)",
          date: "Updated Jan 2026",
        },
      ];
    ...
    const hrTemplates = [
      {
        ...
        code: "OFFER_LETTER",
        desc: "Pre-onboarding offer detailing CTC breakdown, role, designation, and reporting date.",
      },
      {
        name: "Official Appointment Letter",
        code: "APPOINTMENT_LETTER",
        desc: "Post-joining service confirmation, probation terms, and formal employment agreement.",
      },
      {
        name: "Employment Verification Certificate",
        code: "EMPLOYMENT_CERTIFICATE",
        desc: "Official letterhead proof of active employment for bank loans, visas, and tenancy agreements.",
      },
      {
        name: "Official Salary Certificate",
        code: "SALARY_CERTIFICATE",
        desc: "Verified earnings and CTC certificate for financial institutions and credit applications.",
      },
      {
        name: "Relieving & Experience Letter",
        code: "RELIEVING_LETTER",
        desc: "Formal service confirmation stating designation, tenure, and clearance upon exit.",
      },
    ];
  ...
  async openUploadModal() {
    ...
    ModalManager.openModal({
      ...
      subtitle:
        "Attach identity proofs, contracts, education, or certifications",
      contentHtml: `
        ${employeeSelectHtml}
  ...
  async saveUpload() {
    ...
    if (!isEmployee) {
      ...
      if (empSelect) {
        ...
        employeeName =
          empSelect.selectedOptions[0]?.getAttribute("data-name") ||
          employeeName;
      }
    }
  ...
  async openRequestDocumentModal(
    preEmpId = null,
    preEmpName = null,
    preDocName = null,
  ) {
    ...
    if (role === "EMPLOYEE") {
      Toast.warning(
        "Only HR Operations and Admins can dispatch document requests to employees.",
      );
      return;
    }
  ...
  async confirmGenerateLetter(templateCode, templateName) {
    ...
    const signatory = document
      .getElementById("gen-letter-signatory")
      ?.value.trim();
    ...
    try {
      ...
      Toast.success(
        `'${templateName}' generated and saved to ${employeeName}'s dossier!`,
      );
      ModalManager.closeModal();
      Router.mountView("documents");
    } catch (e) {
  ...
  applyFilters() {
    this.currentFilters.search =
      document.getElementById("filter-doc-search")?.value.trim() || "";
    this.currentFilters.categoryCode =
      document.getElementById("filter-doc-cat")?.value || "All";
    this.currentFilters.status =
      document.getElementById("filter-doc-status")?.value || "All";
    Router.mountView("documents");
  },
